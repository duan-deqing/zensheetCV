/**
 * ShapeGrid —— React Bits 开源组件（JSX 源）的 TS 移植版。
 * Canvas 绘制的无限滚动网格背景：方块/六边形/圆/三角形四种瓦片，
 * 支持方向滚动、hover 填色与拖尾淡出；自带 IntersectionObserver +
 * visibilitychange 节能（离屏/切后台自动停帧）。
 * 改造点：补 TS 类型；删除源码中未使用的 ref 与无视觉效果的径向渐变死代码；
 * prefers-reduced-motion 用户降级为静止网格（保留 hover 响应）。
 */
import { useEffect, useRef } from 'react';

type GridShape = 'square' | 'hexagon' | 'circle' | 'triangle';
type GridDirection = 'up' | 'down' | 'left' | 'right' | 'diagonal';
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (event: MouseEvent) => {
      ...
      if (isHex) {
        ...
        // 圆以圆心为基准（round），方格以左上角为基准（floor）
        const offsetX = ((gridOffset.current.x % squareSize) + squareSize) % squareSize;
        const offsetY = ((gridOffset.current.y % squareSize) + squareSize) % squareSize;
        col = Math.round((mouseX - offsetX - squareSize / 2) / squareSize);
        row = Math.round((mouseY - offsetY - squareSize / 2) / squareSize);
      } else {
        const offsetX = ((gridOffset.current.x % squareSize) + squareSize) % squareSize;
        const offsetY = ((gridOffset.current.y % squareSize) + squareSize) % squareSize;
