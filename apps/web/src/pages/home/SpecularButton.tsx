/**
 * SpecularButton —— React Bits 开源组件（JSX 源）的 TS 移植版。
 * WebGL（ogl）渲染的按钮边缘镜面高光：高光角度跟随光标、按指针接近程度淡入，
 * 常驻被动式效果（默认无自动扫光），指针不动时静止，符合 reduced-motion 精神。
 * 改造点：补 TS 类型；样式拆至同目录 CSS；其余逻辑与原组件一致。
 */
...
export default function SpecularButton({
  ...
  const propsRef = useRef<FxProps>({} as FxProps);
  ...
  useEffect(() => {
    ...
    // WebGL 不可用（jsdom 测试环境 / 禁用硬件加速的浏览器）时静默降级：
    // 保留纯 CSS 按钮样式与点击行为，只是没有镜面高光效果
    let cleanup: (() => void) | undefined;
    try {
      cleanup = initSpecularFx(btn, fx);
    } catch {
      /* 降级为无高光 */
    }
    return cleanup;
  ...
  // WebGL 初始化与 rAF 循环（闭包引用 btn/fx/propsRef）；
  // WebGL 不可用时（jsdom / 禁用硬件加速）new Renderer 抛错，由上方 try-catch 捕获降级
  function initSpecularFx(btn: HTMLButtonElement, fx: HTMLElement) {
