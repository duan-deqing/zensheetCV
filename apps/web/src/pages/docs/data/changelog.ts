...
export const STATIC_CHANGELOG: ChangelogEntry[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { zh: 'AI 执行状态面板支持反复展开：面板顶部显示步骤总数，每个步骤可独立展开 / 收起查看完整执行细节（含工具调用的参数与结果摘要、工作流中间步输出），多个步骤可同时展开且互不影响，展开状态在流式更新与重新展开后均保持；行内展示序号、状态点与耗时，长详情以白底卡片滚动查看，错误详情改为浅红底块更醒目', en: 'The AI RUN panel can now be expanded repeatedly: the panel opens with a step-count header, and each step expands / collapses independently to reveal its full execution detail (tool call arguments & result previews, workflow intermediate output) — multiple steps can be open at once without affecting each other, and expansion state survives streaming updates and panel re-opening; rows show index, status dot and timing, long details scroll in a white card, and error details get a more visible light-red block' },
