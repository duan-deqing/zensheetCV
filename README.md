...
<p align="center">
  <img src="https://img.shields.io/badge/%E5%85%8D%E7%99%BB%E5%BD%95%E7%89%88-v0.8.2-2563EB?style=flat-square" alt="Login-free Version" />
...
- **深度工作流**：「全文润色」（诊断 → 起草 → 本地自查 → 修订）与「JD 对齐」（关键词提取 → 覆盖率计算 → 缺口改写）一键串行执行，每步状态实时展示；JD 对齐内置诚实约束，只调整表述不编造经历
- **结构化改写建议**：AI 的修改建议以 diff 确认卡片逐条呈现（原文 / 替换对照），支持单条 / 全部应用与一键撤销本轮，应用后自动保存；AI 不直接修改文档
- **Agent 模式（实验）**：开启后 AI 可调用工具（读取简历全文 / 按节读取 / 查询页数与模板）多轮自主探索，并逐条提议修改（仍需卡片确认）；供应商不支持工具调用时自动降级为建议块模式
- **AI 执行面板**：每条回复上方「AI 执行」可反复展开，逐步骤查看完整执行细节（工具调用参数与结果摘要、工作流中间步输出）与错误详情，可多个步骤同时展开
